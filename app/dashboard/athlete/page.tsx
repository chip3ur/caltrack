...
const GOAL_RECOMMENDATIONS: Record<string, { templateId: string; label: string }> = {
  masse: { templateId: 'ppl-6j', label: 'Pour prendre de la masse, le Push/Pull/Legs est idéal (volume élevé, surcharge progressive).' },
  perte: { templateId: 'cardio-force', label: 'Pour perdre du poids, le programme Cardio + Force maximise la dépense calorique.' },
  maintien: { templateId: 'full-body-3j', label: 'Pour maintenir ta forme, le Full Body 3 jours couvre tous les groupes musculaires efficacement.' },
}

export default function AthletePage() {
  const router = useRouter()

  // Programme coach state
  ...
  const [freeSelectedDay, setFreeSelectedDay] = useState(1)
  const [freeSets, setFreeSets] = useState<FreeSetLog[]>([])
  const [freeSessionRpe, setFreeSessionRpe] = useState<number | ''>('')
  const [freeSessionDuration, setFreeSessionDuration] = useState<number | ''>('')
  const [freeSessionStart, setFreeSessionStart] = useState<Date | null>(null)
  ...
  async function loadData() {
    ...
    const { data: profile } = await supabase.from('profiles').select('role, goal').eq('id', session.user.id).single()
    if (profile?.role) setUserRole(profile.role)
    if (profile?.goal) setUserGoal(profile.goal)

    // Coaches use this page for personal tracking — default to séance libre tab
  ...
  async function saveSession() {
    ...
    if (log) {
      ...
      if (validSets.length > 0) {
        const { data: prData } = await supabase.rpc('get_exercise_prs', { p_athlete_id: session.user.id })
        const prMap: Record<string, number> = {}
        if (prData) prData.forEach((pr: any) => { prMap[pr.exercise_id] = pr.max_weight })
  ...
  function startFreeLogging() {
    if (!selectedTemplate) return
    const day = selectedTemplate.days.find(d => d.day === freeSelectedDay)
    if (!day) return
    const initialSets: FreeSetLog[] = day.exercises.flatMap(ex =>
    ...
    setFreeSets(initialSets)
    setFreeSessionStart(new Date())
    setFreeLogging(true)
  }

  function updateFreeSet(exerciseName: string, setNum: number, field: keyof FreeSetLog, value: number | '') {
    setFreeSets(prev => prev.map(s =>
      s.exercise_name === exerciseName && s.set_number === setNum ? { ...s, [field]: value } : s
    ))
  }
  ...
  async function saveFreeSession() {
    setFreeSaving(true)
    const { data: { session } } = await supabase.auth.getSession()
    if (!session) { setFreeSaving(false); return }

    const autoMinutes = freeSessionStart
      ? Math.round((new Date().getTime() - freeSessionStart.getTime()) / 60000)
      : null

    // Look up exercise IDs by name
    const uniqueNames = [...new Set(freeSets.map(s => s.exercise_name))]
    const { data: exoRows } = await supabase
      .from('exercises')
      .select('id, name')
      .in('name', uniqueNames)

    const exoIdMap: Record<string, string> = {}
    if (exoRows) exoRows.forEach((e: any) => { exoIdMap[e.name] = e.id })

    const { data: log } = await supabase.from('workout_logs').insert({
      athlete_id: session.user.id,
      program_id: null,
      day_number: freeSelectedDay,
      date: new Date().toISOString().split('T')[0],
      duration_min: freeSessionDuration || autoMinutes,
      rpe: freeSessionRpe || null,
      completed: true,
    }).select().single()

    if (log) {
      const validSets = freeSets.filter(s =>
        (s.reps !== '' || s.weight_kg !== '') && exoIdMap[s.exercise_name]
      )
      if (validSets.length > 0) {
        const { data: prData } = await supabase.rpc('get_exercise_prs', { p_athlete_id: session.user.id })
        const prMap: Record<string, number> = {}
        ...
        await supabase.from('workout_sets').insert(
          validSets.map(s => ({
            log_id: log.id,
            exercise_id: exoIdMap[s.exercise_name],
            set_number: s.set_number,
            reps: s.reps || null,
            weight_kg: s.weight_kg || null,
            rpe: s.rpe || null,
            is_pr: s.weight_kg !== '' && s.weight_kg > (prMap[exoIdMap[s.exercise_name]] ?? 0),
          }))
        )
      }
    }
    ...
    setFreeSets([])
    setFreeSessionRpe('')
    setFreeSessionDuration('')
    setTimeout(() => setFreeSaved(false), 3000)
  }

  const dayExercises = exercises.filter(e => e.day_number === selectedDay)
  const recommendation = userGoal ? GOAL_RECOMMENDATIONS[userGoal] : null
        ...
        {tab === 'libre' && (
          <div className="space-y-4">
            {!selectedTemplate ? (
              <>
                ...
                {recommendation && (
                  <div className="p-4 rounded-xl border border-blue-500/20 bg-blue-500/5 text-sm text-blue-200">
                    <span className="font-semibold text-blue-300">💡 Recommandé pour toi — </span>
                    {recommendation.label}
                  </div>
                )}

                <p className="text-sm text-gray-400">Choisis un programme de base :</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {TRAINING_TEMPLATES.map(tpl => (
                    <button key={tpl.id}
                      onClick={() => { setSelectedTemplate(tpl); setFreeSelectedDay(tpl.days[0]?.day ?? 1); setFreeLogging(false) }}
                      className={`p-5 rounded-xl border bg-[var(--bg-card)] hover:border-blue-500/30 hover:bg-blue-500/5 transition-all text-left ${
                        recommendation?.templateId === tpl.id
                      ...
                      {recommendation?.templateId === tpl.id && (
                        <p className="text-xs text-blue-400 mt-2 font-medium">⭐ Recommandé pour ton objectif</p>
                      )}
                    </button>
                  ))}
                </div>
              ...
              /* ── FREE SESSION LOGGING MODE ── */
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 rounded-xl bg-green-500/10 border border-green-500/20">
                  <div className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
                    <span className="text-sm font-semibold text-green-300">
                      {selectedTemplate.name} — {selectedTemplate.days.find(d => d.day === freeSelectedDay)?.label ?? `Jour ${freeSelectedDay}`}
                    </span>
                  </div>
                  <button onClick={() => setFreeLogging(false)} className="text-xs text-gray-500 hover:text-red-400 transition-colors">Annuler</button>
                </div>

                {selectedTemplate.days.find(d => d.day === freeSelectedDay)?.exercises.map(ex => {
                  const exoSets = freeSets.filter(s => s.exercise_name === ex.name)
                  return (
                    <div key={ex.name} className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] overflow-hidden">
                      <div className="px-4 py-3 border-b border-[var(--border)] flex items-center justify-between">
                        <span className="font-semibold text-[var(--text-primary)]">{ex.name}</span>
                        <div className="flex items-center gap-3">
                          {ex.rest_seconds > 0 && (
                            <button onClick={() => setRestTimer({ seconds: ex.rest_seconds })}
                      ...
                      <div className="p-4">
                        ...
                        {exoSets.map(s => (
                          <div key={s.set_number} className="grid grid-cols-4 gap-2 mb-2">
                            ...
                            <input type="number" min={0} value={s.reps}
                              onChange={e => updateFreeSet(ex.name, s.set_number, 'reps', e.target.value ? parseInt(e.target.value) : '')}
                              placeholder="—"
                            ...
                            <input type="number" min={0} step={0.5} value={s.weight_kg}
                              onChange={e => updateFreeSet(ex.name, s.set_number, 'weight_kg', e.target.value ? parseFloat(e.target.value) : '')}
                              placeholder="—"
                              className="px-2 py-2 rounded bg-[var(--bg-input)] border border-[var(--border)] text-sm text-center text-[var(--text-primary)] focus:outline-none focus:border-blue-500/50" />
                            <input type="number" min={1} max={10} value={s.rpe}
                              onChange={e => updateFreeSet(ex.name, s.set_number, 'rpe', e.target.value ? parseInt(e.target.value) : '')}
                              placeholder="—"
                              className="px-2 py-2 rounded bg-[var(--bg-input)] border border-[var(--border)] text-sm text-center text-[var(--text-primary)] focus:outline-none focus:border-blue-500/50" />
                          </div>
                        ))}
                        {ex.notes && <p className="text-xs text-blue-300/60 italic mt-2">💬 {ex.notes}</p>}
                      </div>
                    </div>
                  )
                })}
                ...
                <div className="p-4 rounded-xl border border-[var(--border)] bg-[var(--bg-card)]">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="number" min={0} value={freeSessionDuration}
                        onChange={e => setFreeSessionDuration(e.target.value ? parseInt(e.target.value) : '')}
                        placeholder={freeSessionStart ? `~${Math.round((Date.now() - freeSessionStart.getTime()) / 60000)} min` : 'Ex: 60'}
                    ...
                    <div>
                      ...
                      <input type="number" min={1} max={10} value={freeSessionRpe}
                        onChange={e => setFreeSessionRpe(e.target.value ? parseInt(e.target.value) : '')}
                        placeholder="Ex: 7"
                        className="w-full px-3 py-2 rounded bg-[var(--bg-input)] border border-[var(--border)] text-sm text-[var(--text-primary)] focus:outline-none" />
                    </div>
                  </div>
                ...
                <button onClick={saveFreeSession} disabled={freeSaving}
                  ...
                  {freeSaving ? 'Enregistrement...' : '✓ Terminer la séance'}
                </button>
              </div>
            ) : (
              /* ── TEMPLATE DAY VIEW ── */
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => { setSelectedTemplate(null); setFreeLogging(false) }} className="text-xs text-gray-500 hover:text-gray-300">← Changer</button>
                </div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {selectedTemplate.days.map(day => (
                    <button key={day.day}
                      onClick={() => { setFreeSelectedDay(day.day); setFreeLogging(false) }}
                      className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                        freeSelectedDay === day.day
                ...
                {selectedTemplate.days.filter(d => d.day === freeSelectedDay).map(day => (
                  <div key={day.day}>
                    <p className="text-xs text-gray-500 mb-3 font-medium">{day.label}</p>
                    <div className="space-y-2">
                      {day.exercises.map((ex, i) => (
                        <div key={i} className="flex items-center justify-between p-4 rounded-xl border border-[var(--border)] bg-[var(--bg-card)]">
                          <div>
                    ...
                    <button onClick={startFreeLogging}
                      className="w-full mt-4 py-3 rounded-xl bg-green-600 hover:bg-green-500 text-white font-semibold text-sm transition-colors">
                      🏋️ Démarrer la séance — Jour {day.day}
                    </button>
                  </div>
                ))}
              </div>
                      ...
                      return (
                        <div key={exo.id} className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] overflow-hidden">
                          <div className="px-4 py-3 border-b border-[var(--border)] flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-3">
                              {pr && <span className="text-xs text-yellow-400/80">🏆 PR : {pr}kg</span>}
                              {exo.rest_seconds > 0 && (
                          ...
                          <div className="p-4">
                              ...
                              return (
                                <div key={s.set_number} className={`grid grid-cols-4 gap-2 mb-2 rounded-lg transition-all ${isBeatPR ? 'bg-yellow-500/10 ring-1 ring-yellow-500/30' : ''}`}>
                                  ...
                                  <input type="number" min={0} value={s.reps}
                                    ...
                                    className="px-2 py-2 rounded bg-[var(--bg-input)] border border-[var(--border)] text-sm text-center text-[var(--text-primary)] focus:outline-none focus:border-blue-500/50" />
                                  <input type="number" min={0} step={0.5} value={s.weight_kg}
                                    onChange={e => updateSet(exo.exercise_id, s.set_number, 'weight_kg', e.target.value ? parseFloat(e.target.value) : '')}
                                    placeholder="—"
                            ...
                            {exo.notes && <p className="text-xs text-blue-300/60 italic mt-2">💬 {exo.notes}</p>}
                          </div>
                        </div>
                      )
                    })}
