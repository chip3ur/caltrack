...
export default function AthletePage() {
  ...
  async function saveSession() {
    ...
    const { data: log } = await supabase.from('workout_logs').insert({
      ...
      duration_min: sessionDuration || null,
      rpe: sessionRpe || null,
      completed: true,
    }).select().single()
            ...
            {!logging ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="font-semibold text-[var(--text-primary)]">Séance en cours — Jour {selectedDay}</h2>
                  <button onClick={() => setLogging(false)} className="text-xs text-gray-500 hover:text-gray-300">Annuler</button>
                </div>
                ...
                {dayExercises.map(exo => {
                  ...
                  return (
                    <div key={exo.id} className="p-4 rounded-xl border border-[var(--border)] bg-[var(--bg-card)]">
                      <div className="font-medium text-sm text-[var(--text-primary)] mb-3">{exo.exercise_name}</div>
                ...
                <div className="p-4 rounded-xl border border-[var(--border)] bg-[var(--bg-card)]">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="text-xs text-gray-500 mb-1 block">Difficulté globale (RPE 1-10)</label>
